...
from project_struct.utils import hashed_password, check_password, create_access_token
...
class LoginView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"message": "Invalid Password/Username"}, status=status.HTTP_400_BAD_REQUEST
            )
        return Response(
            {"message": serializer.errors}, status=status.HTTP_400_BAD_REQUEST
        )